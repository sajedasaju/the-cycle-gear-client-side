      ...
      <td className="p-2  text-left block md:table-cell border-x-2">
        <span className="inline-block w-1/3 md:hidden font-bold">Status</span>
        {
          order.paid && order.pandingChange !== "shipped" && (
            <>
              <button
                className="btn btn-xs btn-red-500 mr-2"
        ...
        {!order.paid ? (
          <button disabled className="btn btn-xs btn-red-500 mr-3">
            Unpaid
          </button>
        ) : (
          <button disabled className="btn btn-xs btn-red-500 mr-3">
        ...
        {order.paid && order.pandingChange === "shipped" && (
          <>
            <button disabled className="btn btn-xs btn-red-500">
              Shipped
            </button>
          </>
        )}
      ...
      <td className="p-2  text-left block md:table-cell border-x-2">
        ...
        {!order.paid ? (
          <label
            ...
            className="btn btn-xs btn-error bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border border-red-500 rounded"
          >
            Delete
          </label>
        ) : (
          <button
            disabled
            className="btn btn-xs btn-error bg-red-500 hover:bg-red-700 text-black font-bold py-1 px-2 border border-red-500 rounded"
          >
            Delete
          </button>
        )}
